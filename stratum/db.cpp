...
void db_update_coinds(YAAMP_DB *db)
{
	...
	while((row = mysql_fetch_row(result)) != NULL)
	{
		...
		if(row[22] && row[23]) coind->actual_ttf = std::min(atoi(row[22]), atoi(row[23]));
		else if(row[22]) coind->actual_ttf = atoi(row[22]);
		coind->actual_ttf = std::min(coind->actual_ttf, 120);
		coind->actual_ttf = std::max(coind->actual_ttf, 20);
...
//			continue;
		...
		if(remote->renter)
		{
			if(!strcmp(g_current_algo->name, "sha256"))
				remote->speed = std::min(remote->speed, std::max(remote->renter->balance/g_current_algo->rent*100000000, 1.0));
			else
				remote->speed = std::min(remote->speed, std::max(remote->renter->balance/g_current_algo->rent*100000, 1.0));
		}
	}
